...
func TestGetStickerSetSuccess(t *testing.T) {
	gock.New(telegraph.BaseURL).Get(fmt.Sprintf(telegraph.EndpointGetStickerSet, "token")).Reply(http.StatusOK).JSON(`{
		"ok": true,
		"result": {
			"name": "persik",
			"title": "persik",
...
func TestGetStickerSetError(t *testing.T) {
	gock.New(telegraph.BaseURL).Head(fmt.Sprintf(telegraph.EndpointGetStickerSet, "token")).Reply(http.StatusInternalServerError).JSON("")
	defer gock.Off()
...
func TestGetStickerSetFailed(t *testing.T) {
	gock.New(telegraph.BaseURL).Get(fmt.Sprintf(telegraph.EndpointGetStickerSet, "token")).Reply(http.StatusBadRequest).JSON(`{
		"ok": false,
		"error_code": 400,
